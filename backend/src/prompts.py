...
<CRITICAL_RULE>
你必须严格按以下 JSON 格式回复，不要包含任何其他文字。
禁止输出 Markdown 表格、列表、代码块、自然语言解释或任何与 JSON 无关的内容。
</CRITICAL_RULE>

<FORMAT>
{{"tasks":[{{"title":"任务名称（10字内，突出重点）","intent":"任务要解决的核心问题，用1-2句描述","query":"建议使用的检索关键词"}}]}}
</FORMAT>

<EXAMPLE>
❌ 错误（包含多余文字）：已规划了以下任务：| 任务 | 说明 |...

✅ 正确（仅 JSON）：{{"tasks":[{{"title":"背景调研","intent":"收集主题核心背景","query":"xxx 最新进展"}}]}}
</EXAMPLE>

如果主题信息不足以规划任务，请输出：{{"tasks":[]}}
必要时调用 note 工具记录你的思考过程。
"""
