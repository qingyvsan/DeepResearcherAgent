...
class DeepResearchAgent:
    ...
    def run(self, topic: str) -> SummaryStateOutput:
        ...
        elif self._is_summary_only_tasks(state.todo_items):
            reconstructed = self._reconstruct_tasks_from_events(state)
            if reconstructed:
                logger.info(
                    "Reconstructed %d tasks from tool events (planner returned summary-only)",
                    len(reconstructed),
                )
                state.todo_items = reconstructed

        for task in state.todo_items:
            # Consume the generator so the code inside _execute_task actually runs
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _is_summary_only_tasks(tasks: list[TodoItem]) -> bool:
        """Check if the planner only returned a summary placeholder instead of real tasks."""
        if len(tasks) != 1:
            return False

        title = tasks[0].title
        if any(kw in title for kw in ("规划完成", "plan complete", "done", "planning complete")):
            return True

        query = tasks[0].query
        if any(kw in query for kw in ("无需", "no need", "not required", "无需检索")):
            return True

        return False

    def _reconstruct_tasks_from_events(self, state: SummaryState) -> list[TodoItem] | None:
        """Rebuild the task list from tool call events recorded during planning.

        The planner agent typically creates one note per task via
        ``[TOOL_CALL:note:{"action":"create","task_id":1,...}]`` but
        ``ToolAwareSimpleAgent`` strips those markers from the response text.
        When the remaining text only contains a summary JSON, we reconstruct
        the real tasks from the events already captured by ``_tool_tracker``.
        """
        events = self._tool_tracker.as_dicts()
        note_events: list[dict[str, Any]] = []

        for event in events:
            if event.get("tool") != "note":
                continue
            params = event.get("parsed_parameters")
            if not isinstance(params, dict):
                continue
            if params.get("action") not in ("create", "update"):
                continue
            if params.get("task_id") is None:
                continue
            note_events.append(params)

        if not note_events:
            logger.debug("No note-creation tool events found — cannot reconstruct tasks")
            return None

        seen_ids: set[int] = set()
        tasks: list[TodoItem] = []

        for params in note_events:
            raw_id = params.get("task_id")
            try:
                task_id = int(raw_id)  # type: ignore[arg-type]
            ...
            title = str(params.get("title") or f"任务{task_id}").strip()
            intent = str(params.get("intent") or "聚焦主题的关键问题").strip()
            content = params.get("content") or ""
            query = self._guess_query_from_content(content, title, state.research_topic)

            tasks.append(TodoItem(id=task_id, title=title, intent=intent, query=query))

        if not tasks:
            return None

        tasks.sort(key=lambda t: t.id)
        logger.info("Reconstructed %d tasks from tool events", len(tasks))
        return tasks

    @staticmethod
    def _guess_query_from_content(content: str, title: str, topic: str) -> str:
        """Best-effort query extraction from note content."""
        if not content:
            return f"{topic} {title}" if topic else title
        ...
        for keyword in ("检索方向", "检索关键词", "查询", "query", "搜索"):
            ...
            actual = content[idx:idx + len(keyword)]
            rest = content[idx + len(keyword):].strip().strip("：:，, ")
            if rest:
                return rest
